...
export const DEVICE_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR: any = {
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => DeviceCompleteComponent),
    multi: true
};
	...
	template: `
        <p-autoComplete [(ngModel)]="value" [disabled]="disabled" placeholder="Hint: type to search..." field="name" [suggestions]="suggestions" (completeMethod)="complete($event)" (onSelect)="select($event)">
            <ng-template let-device pTemplate="item">
                <device-line [device]="device"></device-line>
            </ng-template>
        </p-autoComplete>
	`,
	selector: 'device-auto-complete',
    providers: [DEVICE_AUTO_COMPLETE_CONTROL_VALUE_ACCESSOR]
})
export class DeviceCompleteComponent implements ControlValueAccessor {
    @Input() disabled : boolean = false;
    @Input() id : string;
    @Input() name : string;

    //The internal data model
    private _value: Device = null;

    public suggestions : Device[] = [];
    ...
    complete(event:any) {
        this.deviceService.complete(event.query).
            subscribe(
